...
class AppController {
  ...
  registerEventBindings() {
    ...
    window.addEventListener('keydown', (e) => {
      if (e.code === 'Space' && document.activeElement !== this.manualTextInput && this.state.lyricsMode === 'manual') {
        // Prevent window screen jump logic defaults
        e.preventDefault();
    ...
    this.syncEngine.onTypingTrigger = () => {
      this.chatUI.showTypingStatus();
    };
  ...
  setupDropzone(dropzoneElement, hiddenInputElement, fileCallback) {
    ...
    dropzoneElement.addEventListener('dragover', (e) => {
      e.preventDefault();
      dropzoneElement.classList.add('border-emerald-500', 'bg-slate-800');
    });

    dropzoneElement.addEventListener('dragleave', () => {
      dropzoneElement.classList.remove('border-emerald-500', 'bg-slate-800');
    });
  ...
  switchLyricsMode(mode) {
    ...
    if (mode === 'api') {
      this.modeApiBtn.className = 'bg-emerald-600 text-white py-1.5 px-2 rounded-lg text-xs font-semibold hover:bg-emerald-500 transition-colors';
      this.fetchExternalLyrics();
    } else if (mode === 'lrc') {
      this.modeLrcBtn.className = 'bg-teal-600 text-white py-1.5 px-2 rounded-lg text-xs font-semibold hover:bg-teal-500 transition-colors';
  ...
  stampCurrentManualLine() {
    ...
    // Sort chronologically in case user stamps out of order due to manual seeking
    ...
    // Enable export button once lyrics are available
  ...
  updateParsedLyricsWorkspace(lyrics) {
    ...
    if (lyrics.length > 0) {
      ...
      alert("Format verification warning: Parsed structure is empty.");
    }
  }
  ...
  async initiateVideoRecord() {
    ...
    // Restart track context cleanly for uniform audio-video sync alignment from time zero
    this.audioPlayer.currentTime = 0;
    this.chatUI.clear();
    this.syncEngine.resetState();

    // Play track and start capture recording sequence
